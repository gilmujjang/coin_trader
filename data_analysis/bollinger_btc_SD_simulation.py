...
with open(filename, 'r', encoding='utf-8') as f:
    ...
    for d in json_data:
        ...
        bollinger_center_1.append(ten_days_average)

        # 최근 10일간 표준편차
        ten_days_standard_deviation = round(np.std(recent_ten_days_list), 2)

        bollinger_up_1.append(
            ten_days_average+ten_days_standard_deviation*SD_1)
        bollinger_bottom_1.append(
            ten_days_average-ten_days_standard_deviation*SD_1)
        bollinger_up_2.append(
            ten_days_average+ten_days_standard_deviation*SD_2)
        bollinger_bottom_2.append(
            ten_days_average-ten_days_standard_deviation*SD_2)
        bollinger_up_3.append(
            ten_days_average+ten_days_standard_deviation*SD_3)
...
ax.set_title(
    'BTC-볼린저 상단 매수 고가 -10% 매도, 이동평균선 20일, 표준편차 1배,1.5배,2배 비교, 총 자산의 100%투자, 2013~2021 전체기간', fontsize=22)
ax.set_ylabel('수익률 (배)')
ax.set_xlabel('Date')
ax.xaxis.set_major_locator(ticker.FixedLocator(day_list))
ax.xaxis.set_major_formatter(ticker.FixedFormatter(name_list))
plt.plot(date, return_rate_1, date, return_rate_2,
         date, return_rate_3, date, relative_price)
plt.legend(['SD 1배', 'SD 1.5배', 'SD 2배', '보유 수익률'])
plt.show()
